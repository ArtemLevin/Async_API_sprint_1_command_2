...
async def run_person_etl(
    ...
    """
    Запускает ETL процесс для извлечения персон из фильмов и создания индекса персон.

    :param films_index: Имя индекса фильмов в Elasticsearch (по умолчанию 'films').
    :param person_index: Имя индекса персон в Elasticsearch (по умолчанию 'persons').
    ...
    """
    logger.info(
        "Получен запрос на запуск ETL процесса для персон: films_index='%s', person_index='%s'",
        films_index,
        person_index,
    )
    ...
    try:
        # Запуск ETL процесса
        logger.info("Запуск ETL процесса для индексов: films_index='%s', person_index='%s'", films_index, person_index)
        ...
        logger.error("Ошибка при выполнении ETL процесса для персон: %s", str(e))
        raise HTTPException(
